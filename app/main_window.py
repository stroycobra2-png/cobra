import json
import sys, socket, secrets
from pathlib import Path
from PySide6.QtCore import Qt, QObject, Signal, Slot, QThread, QDate, QProcess, QProcessEnvironment
...
class MainWindow(QMainWindow):
    ...
    def __init__(self,db,app):
        ...
        self.mobile_process=None; self.mobile_pin=None; self.mobile_port=5050
    ...
    def show_hamburger_menu(self):
        ...
        menu.addSeparator()
        menu.addAction('📱  Telefon Modu', self.start_mobile_mode)
        menu.addSeparator()
    ...
    def start_mobile_mode(self):
        if self.mobile_process is not None and self.mobile_process.state()!=QProcess.ProcessState.NotRunning:
            self._show_mobile_access_info(); return

        self.mobile_pin=f"{secrets.randbelow(1000000):06d}"
        root=Path(__file__).resolve().parents[1]
        script=root/'mobile_server.py'
        if not script.exists():
            QMessageBox.warning(self,'Telefon Modu','mobile_server.py bulunamadı.'); return

        process=QProcess(self)
        env=QProcessEnvironment.systemEnvironment()
        env.insert('DIL_MOBILE_PIN',self.mobile_pin)
        env.insert('DIL_MOBILE_PORT',str(self.mobile_port))
        process.setProcessEnvironment(env)
        process.setWorkingDirectory(str(root))
        process.setProgram(sys.executable)
        process.setArguments([str(script),'--host','0.0.0.0','--port',str(self.mobile_port)])
        process.setProcessChannelMode(QProcess.ProcessChannelMode.MergedChannels)
        process.start()
        if not process.waitForStarted(5000):
            QMessageBox.warning(self,'Telefon Modu','Mobil sunucu başlatılamadı. KURULUM.bat dosyasını yeniden çalıştırıp mobil paketlerin kurulu olduğundan emin ol.'); return
        self.mobile_process=process
        self.statusBar().showMessage(f'Telefon modu açık • http://{self._local_ip()}:{self.mobile_port}')
        self._show_mobile_access_info()

    def _show_mobile_access_info(self):
        ip=self._local_ip(); pin=self.mobile_pin or '------'
        msg=QMessageBox(self)
        msg.setWindowTitle('📱 Telefon Modu')
        msg.setIcon(QMessageBox.Icon.Information)
        msg.setText('Telefon bağlantısı hazır')
        msg.setInformativeText(
            f'Telefon ve bilgisayar aynı Wi-Fi ağında olsun.\n\n'
            f'Adres:  http://{ip}:{self.mobile_port}\n'
            f'Bağlantı kodu:  {pin}\n\n'
            'Bu adresi telefon tarayıcısında aç. Android Chrome veya iPhone Safari üzerinden Ana Ekrana Ekle seçeneğini kullanabilirsin. '
            'Bilgisayar kapanırsa yerel telefon bağlantısı da kapanır.'
        )
        msg.exec()

    def closeEvent(self,event):
        if self.mobile_process is not None and self.mobile_process.state()!=QProcess.ProcessState.NotRunning:
            self.mobile_process.terminate()
            if not self.mobile_process.waitForFinished(1500):
                self.mobile_process.kill()
