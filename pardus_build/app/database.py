...
import sqlite3, json
from .curriculum import LEVELS, generate_lessons

# Platform-safe persistent user data location.
# Pardus/Linux packages are installed read-only under /opt, so user progress
# lives under XDG_DATA_HOME (normally ~/.local/share).
if sys.platform.startswith('linux'):
    _xdg = Path(os.environ.get('XDG_DATA_HOME', Path.home() / '.local' / 'share'))
    APP_DIR = _xdg / '12d-dil-programi'
    DATA_DIR = APP_DIR
    ...
    def set_setup(self, name, level='A1', daily_goal=20):
        ...
        with self.connect() as c:
            c.execute('UPDATE profiles SET name=?,current_level=?,daily_goal=?,setup_complete=1 WHERE id=?',
                      ((name or '').strip() or 'Öğrenci',level,daily_goal,pid))
            c.commit()
    ...
    def create_class(self, name, level_code='A1', academic_year='', notes=''):
        ...
        with self.connect() as c:
            cur=c.execute('''INSERT INTO classes(owner_profile_id,name,level_code,academic_year,notes,created_at)
                VALUES(?,?,?,?,?,?)''',(pid,name,level_code,(academic_year or '').strip(),(notes or '').strip(),datetime.now().isoformat(timespec='seconds')))
    ...
    def add_class_plan(self, class_id, lesson_id, planned_date='', notes=''):
        ...
        with self.connect() as c:
            cur=c.execute('''INSERT INTO class_lesson_plans(class_id,lesson_id,planned_date,status,notes,created_at)
                VALUES(?,?,?,'Bekliyor',?,?)''',(class_id,lesson_id,(planned_date or '').strip(),(notes or '').strip(),datetime.now().isoformat(timespec='seconds')))
            c.commit(); return cur.lastrowid
