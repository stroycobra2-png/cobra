...
class SpeechService:
    ...
    def speak(self, text):
        # On Pardus/Linux use espeak-ng first. It is installed as a .deb
        # dependency and bundled into the AppImage build when possible.
        if sys.platform.startswith('linux'):
            try:
                exe = shutil.which('espeak-ng') or shutil.which('espeak')
                if exe:
                    subprocess.Popen(
                        [exe, '-v', 'en-us', '-s', str(int(self.rate)), str(text)],
    ...
    def microphone_info(self):
        ...
        try:
            ...
            if default_in is None or int(default_in) < 0:
                return False, 'Varsayılan bir mikrofon seçili değil.'
    ...
    def listen_once(self):
        ...
        try:
            ...
            if default_in is None or int(default_in) < 0:
                return False, (
                    'Varsayılan mikrofon bulunamadı. '
                    'Sistem ses ayarlarından varsayılan giriş aygıtını seç.'
                )
            ...
            # sounddevice/PortAudio device errors land here.
            return False, (
                'Mikrofon kaydı başlatılamadı. Mikrofon iznini ve varsayılan '
